...
scd_control_handle_verify(const char *verify_data_file, const char *verify_sig_file,
			  const char *verify_cert_file, str_t *verify_ca_file,
			  bool ignore_time, const char *hash_algo)
{
	...
	if ((ret = ssl_verify_certificate(verify_cert_file, SSIG_ROOT_CERT, ignore_time)) ==
		0) {
		verified = true;
	...
	// Retry with Local CA
	if ((ret = ssl_verify_certificate(verify_cert_file, LOCALCA_ROOT_CERT,
						ignore_time)) == 0) {
